    ...
    <div className="bg-white h-screen w-72 hidden fixed md:block border-r border-r-slate-200">
      ...
      <div className="flex flex-col text-gray-800 text-base font-semibold pt-3">
        {nav.map((row, index) =>
          row.active ? (
            <Link
              key={index}
              href={row.link}
              className="flex items-center text-md bg-gray-100 text-indigo-700 py-4 pl-6 div-item mb-2"
            >
              {row.icon}
              {row.title}
            </Link>
            ...
            <Link
              key={index}
              href={row.link}
              className="flex items-center text-md  text-gray-500 hover:bg-gray-100 hover:text-indigo-500 py-4 pl-6 div-item mb-2"
            >
              {row.icon}
              {row.title}
            </Link>
        ...
        <div
          ...
          className="flex items-center text-md cursor-pointer text-gray-500 hover:bg-gray-100 hover:text-indigo-500 py-4 pl-6 div-item mb-2"
        >
